...
class CurrentAccount(Account):
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__}: ('{self.agency}', '{self.account_number}', '{self.customer.name}')>"
...
def log_transaction(func):
    def envelope(*args, **kwargs):
        ...
        with open(ROOT_PATH / 'Log.txt', 'a') as arquivo:
            arquivo.write(
                        f"[{date_time}] Função '{func.__name__}' executada com argumermentos {args} e {kwargs}. Retornou {resultado}\n"
                         )
        print(f"{date_time}: {func.__name__.upper()}")
